...
class IMAP_Copy(object):
    source = {"host": "localhost", "port": 993}
    source_auth = ()
    destination = {"host": "localhost", "port": 993}
    destination_auth = ()
    folder_mapping = []
    ...
    def _connect(self, target):
        ...
        if target == "source":
            self._conn_source = connection
        elif target == "destination":
            self._conn_destination = connection
    ...
    def copy(self, source_folder, destination_folder, skip, limit, recurse=True):
        ...
        if status != "OK" and not self.create_folders:
            self.logger.error(
                "Couldn't open destination folder %s" % destination_folder
            )
            sys.exit(2)
        else:
        ...
        for msg_num in data:
            ...
            if progress_count <= skip:
                self.logger.info(
                    "Skipping mail %d of %d" % (progress_count, mail_count)
                )
                continue
            else:
                ...
                internaldate_start = flags_line.index("INTERNALDATE ") + len(
                    "INTERNALDATE "
                )
                ...
                self.logger.info(
                    "Copy mail %d of %d (copy_count=%d, sha1(message)=%s)"
                    % (progress_count, mail_count, copy_count, message_sha1)
                )

                if limit > 0 and copy_count >= limit:
                    self.logger.info(
                        "Copy limit %d reached (copy_count=%d)" % (limit, copy_count)
                    )
                    break

        self.logger.info(
            "Copy complete %s => %s (%d out of %d messages copied)"
                ...
                if d:
                    ...
                    if len(l_resp) >= 3:
                        ...
                        if source_mbox != source_folder:
                            ...
                            dest_mbox = source_mbox.replace(
                                source_folder, destination_folder
                            )
                            self.logger.info(
                                "starting copy of folder %s to %s "
                                % (source_mbox, dest_mbox)
                            )
...
def main():
    ...
    parser.add_argument("source", help="source host, e.g. imap.googlemail.com:993")
    ...
    parser.add_argument(
        "destination", help="destination host, e.g. imap.otherhoster.com:993"
    )

    parser.add_argument(
        "destination_auth",
        metavar="destination-auth",
